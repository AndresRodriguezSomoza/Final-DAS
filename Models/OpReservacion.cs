    ...
    public class OpReservacion
    {
        ...
        public List<Reservacion> ObtenerTodasReservaciones()
        {
            ...
            using (var conexion = _conexionBD.ObtenerConexion())
            {
                ...
                using (var comando = new MySqlCommand(@"
                    SELECT r.*, u.Nombre as UsuarioNombre, h.numero_habitacion, h.precio_noche
                    FROM Reservaciones r
                    INNER JOIN Usuarios u ON r.usuario_id = u.Id
                    INNER JOIN Habitaciones h ON r.habitacion_id = h.id", conexion)) // Tablas en mayúscula
                {
        ...
        public Reservacion ObtenerReservacionPorId(int id)
        {
            using (var conexion = _conexionBD.ObtenerConexion())
            {
                ...
                using (var comando = new MySqlCommand(@"
                    ...
                    WHERE r.id = @Id", conexion)) // Tablas en mayúscula
                {
        ...
        public bool CrearReservacion(Reservacion reservacion)
        {
            using (var conexion = _conexionBD.ObtenerConexion())
            {
                conexion.Open();
                using (var comando = new MySqlCommand(
                    "INSERT INTO Reservaciones (usuario_id, habitacion_id, fecha_entrada, fecha_salida, total_habitacion, estatus) VALUES (@UsuarioId, @HabitacionId, @FechaEntrada, @FechaSalida, @TotalHabitacion, @Estatus)", // Tabla en mayúscula
                    conexion))
                {
        ...
        public bool EditarReservacion(Reservacion reservacion)
        {
            using (var conexion = _conexionBD.ObtenerConexion())
            {
                conexion.Open();
                using (var comando = new MySqlCommand(
                    "UPDATE Reservaciones SET usuario_id = @UsuarioId, habitacion_id = @HabitacionId, fecha_entrada = @FechaEntrada, fecha_salida = @FechaSalida, total_habitacion = @TotalHabitacion, estatus = @Estatus WHERE id = @Id", // Tabla en mayúscula
                    conexion))
                {
        ...
        public bool EliminarReservacion(int id)
        {
            ...
            {
                conexion.Open();
                using (var comando = new MySqlCommand("DELETE FROM Reservaciones WHERE id = @Id", conexion)) // Tabla en mayúscula
                {
                    comando.Parameters.AddWithValue("@Id", id);
                    return comando.ExecuteNonQuery() > 0;
                }
